...
impl Sidecar {
    ...
    pub fn spawn_scheduled_task<F, Fut>(
        &self,
    ...
    async fn start_components(&self) -> Result<Vec<ComponentHandle>> {
        ...
        for component in &handles {
            ...
            if let Err(err) = component
                .start()
                .await
                .wrap_err_with(|| format!("component[{name}] start failed"))
            {
    ...
    async fn stop_components(&self, handles: Vec<ComponentHandle>) -> Result<()> {
        for component in handles.into_iter().rev() {
            ...
            component
                .stop()
                .await
                .wrap_err_with(|| format!("component[{name}] stop failed"))?;
...
mod tests {
    ...
    async fn test_component_lifecycle() -> Result<()> {
        ...
        assert_eq!(component.stop_count.load(Ordering::SeqCst), 1, "组件未停止");
    ...
    async fn test_core_task_handle_cancel() -> Result<()> {
        ...
        assert!(*flag.read().await, "核心任务未启动");

        let cancelled = handle.cancel(Duration::from_millis(100)).await;
        assert!(cancelled, "核心任务取消未在超时内完成");
    ...
    async fn test_spawn_scheduled_task_runs_multiple_times() -> Result<()> {
        ...
        sidecar.spawn_scheduled_task("scheduled_task", Duration::from_millis(10), {
            move || {
        ...
        assert!(*counter.read().await >= 2, "定时任务未多次执行");
    ...
    async fn test_spawn_scheduled_task_handle_cancel() -> Result<()> {
        ...
        let handle = sidecar.spawn_scheduled_task("scheduled_cancel", Duration::from_secs(1), {
            move || {
                let counter = counter_clone.clone();
                async move {
                    let mut guard = counter.write().await;
        ...
        tokio::time::sleep(Duration::from_millis(20)).await;
        let cancelled = handle.cancel(Duration::from_millis(100)).await;
        assert!(cancelled, "定时任务取消未在超时内完成");

        tokio::time::sleep(Duration::from_millis(150)).await;
        assert!(*counter.read().await <= 1, "定时任务在取消后仍继续执行");
    ...
    async fn test_spawn_scheduled_task_handles_error() -> Result<()> {
        ...
        let success_counter = Arc::new(RwLock::new(0u32));
        let error_counter = Arc::new(RwLock::new(0u32));
        let success_counter_clone = success_counter.clone();
        let error_counter_clone = error_counter.clone();

        sidecar.spawn_scheduled_task("scheduled_task_error", Duration::from_millis(10), {
            ...
            move || {
                ...
                async move {
                    if toggle {
                        let mut guard = success_counter.write().await;
                        *guard += 1;
                        Ok(())
                    } else {
                        let mut guard = error_counter.write().await;
                        *guard += 1;
        ...
        let success_count = *success_counter.read().await;
        let error_count = *error_counter.read().await;

        assert!(success_count >= 1, "定时任务成功分支未执行");
        assert!(error_count >= 1, "定时任务错误分支未触发");
