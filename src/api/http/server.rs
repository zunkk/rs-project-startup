...
#[openapi(
    ...
    tags((name = "system", description = "系统相关接口")),
    ...
    async fn start(&self) -> Result<()> {
        ...
        if ipc_file_path.exists() {
            fs::remove_file(&ipc_file_path).await.wrap_err(format!(
                "remove ipc file failed: {}",
                ipc_file_path.display()
            ))?;
        }

        let listener = UnixListener::bind(ipc_file_path.clone()).wrap_err(format!(
            "bind ipc file failed, may be other process is running: {}",
            ipc_file_path.display()
        ))?;
...
#[utoipa::path(
    ...
    path = "/ping",
    params(PingReq),
    responses((status = 200, description = "成功", body = Response<String>))
)]
async fn ping(
    _state: Arc<Core>,
    ctx: Context,
